...
This script performs image morphing between two input images based on user-defined control points.
It creates a sequence of morphed images, an animated GIF, and an MP4 video of the morphing process.

Usage:
    python image_morpher.py --base_image <path_to_base_image> --surrogate_image <path_to_surrogate_image> [options]

Required arguments:
    --base_image        Path to the base image file
    --surrogate_image   Path to the surrogate image file

Optional arguments:
    --num_frames        Number of frames for the morphing sequence (default: 20)
    --output_folder     Folder to save output files (default: 'output')

After running the script:
1. A window will open showing both images side by side.
2. Click on corresponding points in both images alternately (at least 3 pairs).
3. Click the "Process" button to start the morphing process.
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Image Morphing Script v1.0')
    ...
    if args.num_frames <= 0:
        parser.error('--num_frames must be greater than 0.')
    return args

def apply_sepia(input_image, intensity):
    """Apply a sepia filter to the input image with the given intensity."""
...
def process_images(event):
    """Process the images based on the selected points and create the morphing sequence."""
...
def create_animations(base_filename):
    """Create animated GIF and MP4 video from the morphed image sequence."""
    import imageio
    ...
    try:
        ...
        print("Error: ffmpeg was not found. GIF output was created, but MP4 export was skipped.")
...
def clear_points(event):
    """Clear all selected points and reset the images."""
