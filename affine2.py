...
if base_image is None:
    print("Error: base_image.jpg not found or could not be opened.")
    exit()
if surrogate_image is None:
    print("Error: surrogate_image.jpg not found or could not be opened.")
    exit()
...
ax_process = plt.axes([0.4, 0.05, 0.1, 0.075])
btn_process = Button(ax_process, 'Process')

# Add a Clear Points button
ax_clear = plt.axes([0.51, 0.05, 0.1, 0.075])
...
def process_images(event):
    ...
    mask = cv2.cvtColor(aligned_image, cv2.COLOR_BGR2GRAY)
    _, mask = cv2.threshold(mask, 1, 255, cv2.THRESH_BINARY)
    mask_3ch = cv2.merge([mask, mask, mask])  # Convert mask to 3-channel
    ...
    for i in range(num_frames + 1):
        ...
        # Blend the aligned image and base image
        morphed_foreground = cv2.addWeighted(base_image, beta, aligned_image, alpha, 0)

        # Apply sepia filter to the base image with intensity alpha
        adjusted_background = apply_sepia(base_image, alpha)

        # Optionally feather the mask edges
        # Uncomment the following lines to enable feathering
        # mask_blurred = cv2.GaussianBlur(mask.astype(float), (21, 21), 0) / 255.0
        # mask_blurred_3ch = cv2.merge([mask_blurred, mask_blurred, mask_blurred])
        # morphed_image = (morphed_foreground * mask_blurred_3ch + adjusted_background * (1 - mask_blurred_3ch)).astype(np.uint8)

        # Combine the morphed foreground and adjusted background using the mask
        # If feathering is not used
        morphed_image = np.where(mask_3ch == 255, morphed_foreground, adjusted_background)
